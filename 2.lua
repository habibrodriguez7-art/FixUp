...
local blatantv4 = CombinedModules.blatantv4
...
-- Blatant V1
local catBlatantV4 = makeCategory(mainPage, "Blatant 9N Stabil Kayaknya")
makeToggle(catBlatantV4, "Blatant Mode", "BlatantV4.Enabled", function(on) 
    if blatantv4 then if on then blatantv4.Start() else blatantv4.Stop() end end
end)
makeInput(catBlatantV4, "Complete Delay", "BlatantV4.CompleteDelay", 0.05, function(v)
    if blatantv4 then blatantv4.Settings.CompleteDelay = v end
end)
makeInput(catBlatantV4, "Cancel Delay", "BlatantV4.CancelDelay", 0.1, function(v)
    if blatantv4 then blatantv4.Settings.CancelDelay = v end
end)
       ...
       type(Auto9xTotem.Stop) == "function" then
        ...
        if success and totemNames then
            ...
            makeToggle(catAuto9xTotem, "Enable 9x Totem", nil, function(on)
                is9xTotemRunning = on
                if on then
                    pcall(function() Auto9xTotem.Start() end)
                else
                    pcall(function() Auto9xTotem.Stop() end)
                end
            end)
        end
    end
